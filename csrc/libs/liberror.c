...
static int find_field(lua_State *L, int object_index, int level)
{
    ...
    {                                                  //
        ...
        {                                              //
            ...
            if (find_field(L, object_index, level - 1)) // k v k
            {                                          //
...
static int push_global_function_name(lua_State *L, lua_Debug *ar)
{
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, LUA_LOADED_TABLE); // f LUA_LOADED_TABLE
                                                          //
    if (find_field(L, top + 1, 2))                         // f LUA_LOADED_TABLE name
    {                                                     //
...
int errorL_argerror(lua_State *L, int level, int arg, const char *extra_msg)
{
    lua_Debug ar;
    if (!lua_getstack(L, level, &ar))
    {
        return errorL_errorf(L, level, "bad argument #%d (%s)", arg, extra_msg);
    }
    lua_getinfo(L, "n", &ar);
    if (strcmp(ar.namewhat, "method") == 0 && --arg == 0)
    {
        return extra_msg ? errorL_errorf(L, level, "calling '%s' on bad self (%s)", ar.name, extra_msg)
                        : errorL_errorf(L, level, "calling '%s' on bad self", ar.name);
    }
    if (ar.name == NULL)
    {
        ar.name = push_global_function_name(L, &ar) ? lua_tostring(L, -1) : "?";
    }
    return extra_msg ? errorL_errorf(L, level, "bad argument #%d to '%s' (%s)", arg, ar.name, extra_msg)
