...
{
    int arg = (int)luaL_checkinteger(L, 1);
    const char *extra_msg = luaL_optstring(L, 2, NULL);
    int level = (int)luaL_optinteger(L, 3, 1);
    if (level > 0)
    {
 ...
 *
 * Passing `nil` as the custom check function will unregister  custom check.
 *
 * @function register
 ...
 */
static int checks_register_type(lua_State *L)
{
...
/**
 * Unregister the custom check function for a given type descriptor.
 *
 * @function unregister
 * @tparam string descriptor the type descriptor to register a check function for.
 */
static int checks_unregister_type(lua_State *L)
{
    size_t descriptor_len;
    const char *descriptor = luaL_checklstring(L, 1, &descriptor_len);
    ...
    // do nothing if the checkers table is not initialized
    if (checkers_ref == LUA_NOREF) return 0;
...
extern int luaopen_std_checks(lua_State *L)
{
    // clang-format off
