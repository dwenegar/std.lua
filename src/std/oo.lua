...
-- @treturn table a table representing the new mixin.
function mixin(name)
...
local function class_tostring(class)
  ...
  return name
    ...
    or ('anonymous class: 0x%x'):format(hash.hash(class))
end
...
-- local foo = Foo()
--
-- @tparam[opt] string name the name of the new class.
-- @tparam[opt] table super the parent of the new class.
-- @tparam[opt] table ... the mixins for the new class.
-- @treturn table a table representing the new class.
-- @raise if different mixins define members with the same name but different types.
...
--- Indicates whether the given value is a class.
-- @param v the value to test.
...
--- Indicates whether a given class  has the specified parent among its ancestors.
-- @tparam table class the class to test.
-- @tparam table parent the class to compare with the ancestors of the given class.
...
--- Indicates whether a given value is an instance of the specified class.
-- @param v the value to test.
-- @tparam table class the class to compare with the value's class.
...
-- @treturn table a table representing  the class of the given value, or `nil` if the value is not an object.
function class_of(v)
...
-- @tparam table class the class to get the parent class of.
-- @treturn table a table representing the parent of the given class, otherwise `nil`.
...
-- @tparam table class_or_mixin the type to get the name of.
-- @treturn string the name of the given type, or `nil` if the input argument is `nil`.
-- @raise if the given argument is not a class or a mixin or `nil`.
...
function type_of(v)
  ...
  if r == 'table' then
    local mt = getmetatable(v)
    return mt and rawget(mt, '__type') or r
  end
  return r
end

--- Indicates whether the given value responds to the specified method call.
...
--- Indicates whether the given value is object-like.
--
...
--- Indicates whether the given class includes the specified mixin.
-- @tparam table class the class to test.
-- @tparam table mixin the mixin to test for.
...
checks.register_type('object', is_object)
